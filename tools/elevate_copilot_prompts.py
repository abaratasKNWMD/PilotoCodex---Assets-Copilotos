...
CONNECTOR_ENV_KEYS = {
    ...
    "sonarqube_mcp": "SONARQUBE_TOKEN",
    "confluence_mcp_optional": "CONFLUENCE_TOKEN_OPTIONAL",
}
...
def architecture_runtime_section(copilot: dict) -> str:
    if not is_architecture_decision_copilot(copilot):
        return ""
    artifact_ref = runtime_markdown_artifact_ref(ARCHITECTURE_AUDIT_ARTIFACT)
...
def design_runtime_section(copilot: dict) -> str:
    if not is_design_boundary_copilot(copilot):
        return ""
    artifact_ref = runtime_markdown_artifact_ref(DESIGN_AUDIT_ARTIFACT)
    return clean_template(f"""\
...
def build_runtime_section(copilot: dict) -> str:
    if not is_build_implementation_copilot(copilot):
        return ""
    artifact_ref = runtime_markdown_artifact_ref(BUILD_AUDIT_ARTIFACT)
    return clean_template(f"""\
...
def cloud_runtime_section(copilot: dict) -> str:
    if not is_cloud_migration_copilot(copilot):
        return ""
    artifact_ref = runtime_markdown_artifact_ref(CLOUD_AUDIT_ARTIFACT)
    return clean_template(f"""\
...
def runtime_markdown_artifact_ref(artifact: str) -> str:
    if artifact.startswith("shared/"):
        return f"../{artifact}"
    return artifact


def runtime_adapter_files(copilot_id: str) -> dict[str, str]:
    return {
...
def github_profile_path(copilot_id: str) -> str:
    return f"dist/copilots/{copilot_id}/{GITHUB_PROFILE_FILE}"


def github_mcp_placeholders_path(copilot_id: str) -> str:
    return f"dist/copilots/{copilot_id}/{GITHUB_MCP_PLACEHOLDERS_FILE}"


def github_runtime_equivalence(copilot_id: str) -> dict:
    return {
        "sourceOfTruth": f"dist/copilots/{copilot_id}/shared/spec.json",
        "runtimes": RUNTIMES,
        "adapterFiles": runtime_adapter_files(copilot_id),
        "maxUnexplainedDrift": 0,
    }


def codex_tool_protocol_contract(copilot: dict, contract: dict) -> dict:
    cid = copilot["id"]
    schema_ref = f"dist/copilots/{cid}/shared/output_schema.json"
    runtime_file = f"dist/copilots/{cid}/codex/AGENT.md"
    return {
        ...
        "taskPrompt": {
            "artifact": runtime_file,
            "schemaRef": schema_ref,
            "outputSchemaSha256": stable_json_digest(contract["outputSchema"]),
            "requiredSections": CODEX_REQUIRED_SECTIONS,
            "promptSourceRefs": [
                f"dist/copilots/{cid}/shared/spec.json#/systemPrompt",
                f"dist/copilots/{cid}/shared/spec.json#/developerPrompt",
                schema_ref,
            ],
        },
        "localToolProtocol": {
        ...
        "runtimeEquivalence": github_runtime_equivalence(cid),
        "costControl": {
            "promptExpansion": "reference_only_no_schema_duplication",
            "deterministicPythonFirst": True,
            "llmEscalation": "only_after_local_tool_protocol_cannot_decide",
        },
    }


def claude_project_instructions_contract(copilot: dict, contract: dict) -> dict:
    cid = copilot["id"]
    schema_ref = f"dist/copilots/{cid}/shared/output_schema.json"
    runtime_file = f"dist/copilots/{cid}/claude/AGENT.md"
    return {
        ...
        "projectInstructions": {
            ...
            "schemaRef": schema_ref,
            "outputSchemaSha256": stable_json_digest(contract["outputSchema"]),
            "requiredSections": CLAUDE_REQUIRED_SECTIONS,
            "promptSourceRefs": [
                f"dist/copilots/{cid}/shared/spec.json#/systemPrompt",
                f"dist/copilots/{cid}/shared/spec.json#/developerPrompt",
                schema_ref,
        ...
        "agentCard": {
            ...
            "sdlcPhases": copilot["sdlc_phases"],
            "declaredOutputs": copilot["outputs"],
            "sourceOfTruth": f"dist/copilots/{cid}/shared/spec.json",
            "outputSchema": schema_ref,
            "runtimeFile": runtime_file,
            "handoffTargets": ["codex", "github-copilot"],
            "focus": "long_context_synthesis_and_review_ready_project_instructions",
            "avoid": [
                "inventing_file_state",
                "claiming_connector_access_without_evidence",
                "broad_prompt_expansion_without_python_first_evidence",
            ],
            "must": [
                "embed_shared_system_and_developer_prompts_verbatim",
                "cite_evidence_refs_before_recommendations",
                "produce_executor_ready_handoff_when_code_edits_are_needed",
                "use_placeholders_only_for_credentials_billing_and_customer_data",
            ],
        },
        ...
        "costControl": {
            "promptExpansion": "reference_only_no_schema_duplication",
            "deterministicPythonFirst": True,
            "llmEscalation": "only_after_python_evidence_pack_or_ambiguous_tradeoff",
        },
    }


def codex_injection_contract(copilot_id: str) -> dict:
    return {
        "artifact": f"dist/copilots/{copilot_id}/{CODEX_PROTOCOL_FILE}",
        "ownerAgent": CODEX_OWNER_AGENT,
        "mission": CODEX_MISSION,
        "requiredEvidence": CODEX_REQUIRED_EVIDENCE,
        "qualityGates": CODEX_QUALITY_GATES,
        "runtimeEquivalence": {
            "sourceOfTruth": f"dist/copilots/{copilot_id}/shared/spec.json",
            "runtimes": RUNTIMES,
            "traceSource": f"generated/runtime-injection-map.json#/copilots/{copilot_id}/codexAdapterProtocol",
            "maxUnexplainedDrift": 0,
        },
    }


def claude_injection_contract(copilot_id: str) -> dict:
    return {
        "artifact": f"dist/copilots/{copilot_id}/{CLAUDE_PROTOCOL_FILE}",
        "ownerAgent": CLAUDE_OWNER_AGENT,
        "mission": CLAUDE_MISSION,
        "requiredEvidence": CLAUDE_REQUIRED_EVIDENCE,
        "qualityGates": CLAUDE_QUALITY_GATES,
        "runtimeEquivalence": {
            "sourceOfTruth": f"dist/copilots/{copilot_id}/shared/spec.json",
            "runtimes": RUNTIMES,
            "traceSource": f"generated/runtime-injection-map.json#/copilots/{copilot_id}/claudeAdapterProtocol",
            "maxUnexplainedDrift": 0,
        },
        "projectInstructionsRef": f"dist/copilots/{copilot_id}/{CLAUDE_PROTOCOL_FILE}#/projectInstructions",
        "agentCardRef": f"dist/copilots/{copilot_id}/{CLAUDE_PROTOCOL_FILE}#/agentCard",
    }


def release_scorecard_ref(copilot_id: str) -> str:
    return f"{RELEASE_SCORECARD_INDEX}/{copilot_id}"


def release_injection_contract(copilot_id: str) -> dict:
    return {
...
def release_gate_contract() -> dict:
    return {
        ...
        "claimPolicy": "no_product_artifact_claim_without_build_render_or_package_evidence",
...
def github_profile_contract(copilot: dict, contract: dict) -> dict:
    cid = copilot["id"]
    schema_ref = f"dist/copilots/{cid}/shared/output_schema.json"
    runtime_file = f"dist/copilots/{cid}/github-copilot/copilot-agent.md"
    return {
        ...
        "profileDoc": {
            "artifact": runtime_file,
            "profileArtifact": github_profile_path(cid),
            "schemaRef": schema_ref,
            "outputSchemaSha256": stable_json_digest(contract["outputSchema"]),
            "requiredSections": GITHUB_REQUIRED_SECTIONS,
            "promptSourceRefs": [
                f"dist/copilots/{cid}/shared/spec.json#/systemPrompt",
                f"dist/copilots/{cid}/shared/spec.json#/developerPrompt",
                schema_ref,
        ...
        "mcpPlaceholderRef": github_mcp_placeholders_path(cid),
        "repositoryWorkflow": {
            "evidenceOrder": [
                "issue_or_pr_context",
            ...
            "branchPolicyRespected": True,
            "reviewCommentsActionable": True,
            "offlineFallback": "use_local_files_and_mark_connector_evidence_pending",
        },
...
def connector_placeholder(connector: str, copilot_env_keys: list[str]) -> dict:
    env_key = CONNECTOR_ENV_KEYS.get(connector)
    if env_key not in copilot_env_keys and copilot_env_keys:
        env_key = copilot_env_keys[0]
    return {
        "connector": connector,
        "enabled": False,
        "env": env_key,
        "envReference": f"${{{env_key}}}" if env_key else "",
        "credentialValue": "",
...
def github_mcp_placeholders(copilot: dict) -> dict:
    cid = copilot["id"]
    return {
        "policyVersion": GITHUB_PROTOCOL_VERSION,
        "ownerAgent": GITHUB_OWNER_AGENT,
        ...
        "connectors": {
            connector: connector_placeholder(connector, copilot["env_keys"])
            for connector in copilot["connectors"]
        },
        "safeUsage": {
...
def github_injection_contract(copilot_id: str) -> dict:
    return {
        "artifact": github_profile_path(copilot_id),
        "profileDoc": f"dist/copilots/{copilot_id}/github-copilot/copilot-agent.md",
        "mcpPlaceholders": github_mcp_placeholders_path(copilot_id),
        "policyRef": "config/mcp-connectors.example.json",
        "ownerAgent": GITHUB_OWNER_AGENT,
        "mission": GITHUB_MISSION,
        "requiredEvidence": GITHUB_REQUIRED_EVIDENCE,
        "qualityGates": GITHUB_QUALITY_GATES,
        "runtimeEquivalence": github_runtime_equivalence(copilot_id),
    }


def codex_markdown_protocol_section() -> str:
    return clean_template(f"""\
...
def claude_markdown_protocol_section() -> str:
    return clean_template(f"""\

    ## Claude Project Instructions

    - Project instruction artifact: `../shared/claude_project_instructions.json`
    - shared_spec: `../shared/spec.json`
    - agent_card: `../shared/claude_project_instructions.json#/agentCard`
    - evidence_pack_handoff: required before executor transfer.
    - python_first: deterministic catalog, schema and routing checks precede synthesis.
    - traceability_and_cost: reference shared artifacts instead of expanding prompt context repeatedly.
    """)


def github_markdown_profile_section(copilot: dict) -> str:
    return clean_template(f"""\
    ...
    - Owner agent: `{GITHUB_OWNER_AGENT}`
    - Mission: {GITHUB_MISSION}
    - Required evidence: {", ".join(GITHUB_REQUIRED_EVIDENCE)}
    - Quality gates: {", ".join(GITHUB_QUALITY_GATES)}
    ...
    - Connector env names only: {", ".join(copilot['env_keys'])}
    - Connector placeholders: {", ".join(copilot['connectors'])}
...
def elevate(root: Path | None = None) -> None:
    ...
    for copilot in copilots:
        ...
        write_json(base / CODEX_PROTOCOL_FILE, codex_tool_protocol_contract(copilot, contract))
        write_json(base / CLAUDE_PROTOCOL_FILE, claude_project_instructions_contract(copilot, contract))
        write_json(base / GITHUB_PROFILE_FILE, github_profile_contract(copilot, contract))
        write_json(base / GITHUB_MCP_PLACEHOLDERS_FILE, github_mcp_placeholders(copilot))
        ...
        injection_map["copilots"][cid] = {
            ...
            "codexAdapterProtocol": codex_injection_contract(cid),
            "claudeAdapterProtocol": claude_injection_contract(cid),
            "githubCopilotProfileProtocol": github_injection_contract(cid),
        }
        if is_design_boundary_copilot(copilot):
        ...
        if cid in RELEASE_TARGET_COPILOTS:
            injection_map["copilots"][cid]["releaseReadinessAudit"] = release_injection_contract(cid)
        prompt_stats.append({
            "id": cid,
...
def control_room_contract() -> dict:
    return {
        ...
        "releaseTruthGate": {
            ...
            "residualRiskMustBeExplicit": True,
            "releaseReadinessAudit": release_gate_contract(),
        },
    }
...
def render_markdown_agent(copilot: dict, contract: dict, runtime: str) -> str:
    ...
    if runtime == "codex":
        runtime_protocol_section = codex_markdown_protocol_section()
    elif runtime == "claude":
        runtime_protocol_section = claude_markdown_protocol_section()
    elif runtime == "github-copilot":
        runtime_protocol_section = github_markdown_profile_section(copilot)
    sdlc_rows = "\n".join(
...
def render_langchain_agent(copilot: dict, contract: dict) -> str:
    ...
    # Prefer the shared runtime safety module shipped with this workspace adapter.
    ...
    class {class_name}:
        ...
        def audit(self, request: str, evidence: dict[str, Any] | None = None) -> dict[str, Any]:
            ...
            words = self.normalize(request)
            s = words & {{"auth","connector","connectors","credential","credentials","prod","production","release","secret","secrets","security","token","tokens","vulnerability"}}
            w = words & {{"activate","change","changes","commit","delete","deploy","enable","fix","implement","merge","modify","patch","publish","push","remove","rotate","store","update","write","writes"}}
            if s and w:
...
def output_schema(copilot: dict) -> dict:
    ...
    def contains_by_kind(evidence_kind: str) -> dict:
        return {
            "contains": {
                "type": "object",
                "required": ["kind"],
    ...
    def merge_contains(existing: list[dict], required_kinds: list[str]) -> list[dict]:
        seen = {
            item.get("contains", {})
            .get("properties", {})
            .get("kind", {})
        ...
        for evidence_kind in required_kinds:
            if evidence_kind not in seen:
                merged.append(contains_by_kind(evidence_kind))
    ...
    if is_design_boundary_copilot(copilot):
        ...
        evidence_schema = {
            **evidence_schema,
            ...
            "allOf": [contains_by_kind(evidence_kind) for evidence_kind in DESIGN_REQUIRED_EVIDENCE],
            "items": {
    ...
    if is_build_implementation_copilot(copilot):
        ...
        evidence_schema = {
            **evidence_schema,
            ...
            "allOf": merge_contains(evidence_schema.get("allOf", []), BUILD_REQUIRED_EVIDENCE),
            "items": {
                **evidence_schema["items"],
                "properties": {
                    **evidence_schema["items"]["properties"],
    ...
    if is_cloud_migration_copilot(copilot):
        ...
        evidence_schema = {
            **evidence_schema,
            ...
            "allOf": merge_contains(evidence_schema.get("allOf", []), CLOUD_REQUIRED_EVIDENCE),
            "items": {
                **evidence_schema["items"],
                "properties": {
                    **evidence_schema["items"]["properties"],
...
def render_injection_map_md(injection_map: dict) -> str:
    ...
    for cid, item in injection_map["copilots"].items():
        github_protocol = item.get("githubCopilotProfileProtocol", {})
        rows.append(
            f"| {cid} | {item['name']} | {item['runtimeFiles']['codex']} | {item['runtimeFiles']['github-copilot']} | {github_protocol.get('artifact', '')} | {github_protocol.get('mcpPlaceholders', '')} | {item['runtimeFiles']['langchain']} |"
        )
