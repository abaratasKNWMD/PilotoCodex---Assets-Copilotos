# Copiloto Moonshine - Claude Agent V2

## Runtime Injection

- Copilot ID: `moonshine`
- Runtime: `claude`
- Source of truth: `../shared/spec.json`
- Output schema: `../shared/output_schema.json`
- Runtime equivalence: Codex, Claude, GitHub Copilot and LangChain share the same schema, evidence gates, validation commands and connector policy; runtime-specific sections adapt tooling only.
...
|---|---|---|---|
| build | Create scoped implementation plans or patches with affected files and rollback notes. | affected-file diff summary, lint/test command discovery | build artifact plus one validation signal for Copiloto Moonshine |
| test | Turn requirements and code risks into unit, integration, negative and regression tests. | test matrix expansion, pairwise/negative case generation | test artifact plus one validation signal for Copiloto Moonshine |
| operate | Define monitoring, incidents, runbooks, ownership and continuous improvement loops. | runbook and alert coverage check | operate artifact plus one validation signal for Copiloto Moonshine |
...
```json
{
  ...
  "title": "Copiloto Moonshine Output Contract",
  "type": "object",
  "required": [
"copilot_id",
"decision",
...
| Output contract | Produces one of: backend_patch_plan, runtime_diagnostics. | Returns generic consultancy text with no artifact. |
...
- The request enters a phase outside `Copiloto Moonshine` ownership.
