...
- Runtime equivalence contract: `../shared/spec.json#/runtimeEquivalenceContract`
...
- Runtimes covered in fixed order: codex, claude, github-copilot, langchain.
- Shared schema: `../shared/output_schema.json`; do not add runtime-only response fields.
- Shared invariants: Python or structured checks run before judgement, evidence is refs/summaries/digests only, connector use is declaration-only without repo credentials, and unexplained drift must stay at 0.
- Runtime-specific behavior may change only tool invocation syntax. If a runtime cannot perform a step, return a blocker or handoff in the shared schema rather than changing the contract.
