...
- Prompt parity: shared system and developer prompts remain the source of truth; runtime-specific guidance only adds adapter tooling or safety handling.
- Schema parity: every runtime emits the same output schema fields and the same `handoff.next_runtime` enum.
- Cost and traceability: Python-first deterministic checks, targeted evidence instead of repository dumps, cited evidence refs and validation commands.
