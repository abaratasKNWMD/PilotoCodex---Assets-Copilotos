# Copiloto Firefly v6 - Claude Agent V2

## Runtime Injection

- Copilot ID: `firefly_v6`
- Runtime: `claude`
...
## Runtime Equivalence Guard

- Canonical contract: `../shared/spec.json`; machine schema: `../shared/output_schema.json`.
- Required runtimes: Codex, Claude, GitHub Copilot and LangChain with `maxUnexplainedDrift = 0`.
- Reports and matrices store paths, digests and blocker summaries, not prompt bodies.
- Do not expand repository context into prompts when Python can emit a file list, schema check, score or digest.
- Every artifact must include `runtime_trace.source_of_truth`, `runtime_trace.runtime_adapters` and validation commands.
...
|---|---|---|---|
| build | Create scoped implementation plans or patches with affected files and rollback notes. | affected-file diff summary, lint/test command discovery | build artifact plus one validation signal for Copiloto Firefly v6 |
| test | Turn requirements and code risks into unit, integration, negative and regression tests. | test matrix expansion, pairwise/negative case generation | test artifact plus one validation signal for Copiloto Firefly v6 |
| devops | Inspect pipelines, logs, caches, environments, build reproducibility and release safety. | workflow YAML parse, failed job log summarization | devops artifact plus one validation signal for Copiloto Firefly v6 |
...
The machine-readable contract is `dist/copilots/firefly_v6/shared/output_schema.json` (runtime-local link: `../shared/output_schema.json`). It requires `implementation` and `kb_partition` with evidence kinds `implementation_plan`, `stack_rules`, `affected_files`, `validation`, `kb_partition_map`, `source_of_truth_registry`, `context_window_budget` and `runtime_trace`. Runtime adapters must reference this shared schema instead of embedding full copies.
...
| Output contract | Produces one of: kb_partition_report, test_plan, quality_gate. | Returns generic consultancy text with no artifact. |
...
- The request enters a phase outside `Copiloto Firefly v6` ownership.
