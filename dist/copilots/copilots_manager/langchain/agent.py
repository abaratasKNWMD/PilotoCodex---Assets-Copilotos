"""LangChain-compatible Python brain for Copilots Manager.
...
RUNTIME_PARITY_CONTRACT = {
    ...
    "runtimeMap": "generated/runtime-injection-map.json#/copilots/copilots_manager",
    "promptBodiesStored": False,
    "maxUnexplainedDrift": 0,
    "costPolicy": "python_first_llm_sparse; use paths, summaries and digests; no raw repository dump",
    "missingConnectorBehavior": "offline audit with connector evidence marked pending",
}
SDLC_PLAYBOOK = [{"exitEvidence": "discovery artifact plus one validation signal for Copilots Manager", "goal": "Find source systems, owners, constraints, risks and existing documents.", "phase": "discovery", "pythonCheck": "file inventory, connector declarations, source map generation"}, {"exitEvidence": "operate artifact plus one validation signal for Copilots Manager", "goal": "Define monitoring, incidents, runbooks, ownership and continuous improvement loops.", "phase": "operate", "pythonCheck": "runbook and alert coverage check"}]
QUALITY_RUBRIC = [{"criterion": "Evidence first", "failSignal": "Advice appears before evidence or invents repository state.", "passSignal": "Claims cite files, logs, catalog entries, connector outputs or explicit user constraints."}, {"criterion": "Python first", "failSignal": "LLM is asked to count, route, validate keys or scan secrets.", "passSignal": "Deterministic checks are delegated to scripts or structured validation."}, {"criterion": "Output contract", "failSignal": "Returns generic consultancy text with no artifact.", "passSignal": "Produces one of: copilot_registry, routing_plan."}, {"criterion": "Scope control", "failSignal": "Expands beyond copilot mission without handoff.", "passSignal": "Names affected phases, files, connectors and owners."}, {"criterion": "Primary gate", "failSignal": "Moves forward without the primary quality gate.", "passSignal": "Every handoff must include intent, selected copilot, excluded copilots, evidence pack and stop condition."}]
...
class CopilotsManagerAgent:
    ...
    def plan(self, request: str, evidence: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        plan = {
            ...
            "runtime_parity_contract": self.runtime_contract(),
            "audit": audit,
        }
...
def build_agent() -> CopilotsManagerAgent:
    return CopilotsManagerAgent()
