...
- Canonical contract: `../shared/spec.json#/runtimeEquivalenceContract`.
- Shared schema: `../shared/output_schema.json`; `cloud_migration` stays required.
- Required evidence kinds: `migration_inventory`, `target_platform`, `modernization_increments`, `validation`.
- Codex, Claude, GitHub Copilot and LangChain may differ only in adapter mechanics. Prompt source refs, schema, connector placeholders, validation commands and cost guard must stay identical.
- LLM escalation stays sparse: deterministic Python checks run first, then only targeted evidence summaries go to judgement-heavy synthesis.
- Required verification commands: `python tools/validate_copilot_factory.py`, `python tools/validate_prompt_quality.py`, `python tools/validate_runtime_equivalence.py`.
