# Copiloto DevEx - Claude Agent V2
...
- Contract version: `devex-runtime-parity-1.0`.
- Source of truth: `../shared/spec.json#/runtimeParityContract`; schema: `../shared/output_schema.json`.
- Parity target: Codex, Claude, GitHub Copilot and LangChain keep `maxUnexplainedDrift = 0`.
- Cost rule: run deterministic Python checks before LLM synthesis, and pass references or compact evidence packs instead of whole repos or duplicated prompt bodies.
- Trace rule: every result carries source refs, owner, action, scope and validation; missing connector evidence is marked pending, never invented.

## SDLC Playbook

| Phase | Goal | Python Check | Exit Evidence |
|---|---|---|---|
| discovery | Find source systems, owners, constraints, risks and existing documents. | file inventory, connector declarations, source map generation | discovery artifact plus one validation signal for Copiloto DevEx |
| build | Create scoped implementation plans or patches with affected files and rollback notes. | affected-file diff summary, lint/test command discovery | build artifact plus one validation signal for Copiloto DevEx |
| test | Turn requirements and code risks into unit, integration, negative and regression tests. | test matrix expansion, pairwise/negative case generation | test artifact plus one validation signal for Copiloto DevEx |
| release | Prepare versioning, changelog, risk signoff, deployment and rollback evidence. | manifest, changelog and version compatibility check | release artifact plus one validation signal for Copiloto DevEx |
...
```json
{
  ...
  "title": "Copiloto DevEx Output Contract",
  "type": "object",
  "required": [
"copilot_id",
"decision",
...
| Output contract | Produces one of: devex_scorecard, standards_patch_plan. | Returns generic consultancy text with no artifact. |
...
- The request enters a phase outside `Copiloto DevEx` ownership.
