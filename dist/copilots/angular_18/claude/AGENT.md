# Copiloto Angular 18 - Claude Agent V2

## Runtime Injection

- Copilot ID: `angular_18`
- Runtime: `claude`
...
- Version: `angular-18-runtime-contract-1.0`
- Source of truth: `../shared/spec.json`
- Output schema: `../shared/output_schema.json`
- Runtime set: `codex`, `claude`, `github-copilot`, `langchain`
- Prompt invariant: all runtimes embed `systemPrompt` and `developerPrompt` from the shared spec verbatim; runtime-specific guidance can narrow adapter behavior but must not weaken shared behavior.
- Schema invariant: outputs must satisfy `../shared/output_schema.json` and include trace fields: `copilot_id`, `phase`, `expected_outputs`, `evidence`, `actions`, `validation`, `risks`, `handoff`, `implementation`.
- Evidence invariant: judgement follows local files, connector outputs or explicit user constraints; missing connector evidence is marked pending.
- Cost invariant: Python handles deterministic routing, schema and regression checks; LLM context is limited to targeted evidence bundles, never full repository dumps.
- Safety invariant: environment variable names only, placeholders only, no credential values and no connector activation without approval.
- Validation gate: `python tools/validate_copilot_factory.py`, `python tools/validate_prompt_quality.py`, `python tools/validate_runtime_equivalence.py`
...
|---|---|---|---|
| design | Define contracts, modules, UX/API boundaries, data shape and acceptance criteria. | contract schema check, acceptance criteria completeness | design artifact plus one validation signal for Copiloto Angular 18 |
| build | Create scoped implementation plans or patches with affected files and rollback notes. | affected-file diff summary, lint/test command discovery | build artifact plus one validation signal for Copiloto Angular 18 |
| test | Turn requirements and code risks into unit, integration, negative and regression tests. | test matrix expansion, pairwise/negative case generation | test artifact plus one validation signal for Copiloto Angular 18 |
...
```json
{
  ...
  "title": "Copiloto Angular 18 Output Contract",
  "type": "object",
...
| Output contract | Produces one of: frontend_patch_plan, component_test_plan. | Returns generic consultancy text with no artifact. |
...
- The request enters a phase outside `Copiloto Angular 18` ownership.
