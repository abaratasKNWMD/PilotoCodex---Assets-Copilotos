# Copiloto de Discovery AS-IS - Claude Agent V2

## Runtime Injection

- Copilot ID: `as_is_discovery`
- Runtime: `claude`
...
Runtime parity rule:
- All adapters use the shared spec/output_schema, emit the same required fields, keep unexplained drift at 0 and mark tool gaps as pending evidence.
...
|---|---|---|---|
| discovery | Find source systems, owners, constraints, risks and existing documents. | file inventory, connector declarations, source map generation | discovery artifact plus one validation signal for Copiloto de Discovery AS-IS |
| as_is | Document current behavior, interfaces, data flows and business rules before changing anything. | schema extraction, endpoint inventory, dependency graph | as_is artifact plus one validation signal for Copiloto de Discovery AS-IS |
...
The preferred machine-readable shape starts with this core envelope. The full contract is `../shared/output_schema.json`; include the `runtime_contract` object from that schema whenever runtime parity is part of the deliverable.

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "title": "Copiloto de Discovery AS-IS Output Contract",
  "type": "object",
  "required": [
"copilot_id",
"decision",
...
"runtime_contract": {
  "$ref": "../shared/output_schema.json#/properties/runtime_contract"
}
  }
}
```
...
| Output contract | Produces one of: as_is_report, inventory_json. | Returns generic consultancy text with no artifact. |
...
- Source contract: `../shared/spec.json` plus `../shared/output_schema.json`.
- Runtime order: codex, claude, github-copilot, langchain.
- Max unexplained drift: `0`; route cheap checks through Python first and mark connector/tool gaps as pending evidence.

## Escalation

Escalate to another copilot when:

- The request enters a phase outside `Copiloto de Discovery AS-IS` ownership.
