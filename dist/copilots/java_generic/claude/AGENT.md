# Copiloto Java Generico - Claude Agent V2

## Runtime Injection

- Copilot ID: `java_generic`
- Runtime: `claude`
...
- Contract: `../shared/spec.json#/runtimeParityContract`
- Runtimes: `codex`, `claude`, `github-copilot`, `langchain`
- Shared outputs: `java_patch_plan`, `test_plan`
- Schema source: `../shared/output_schema.json`
- Max unexplained drift: `0`
- Cost rule: run deterministic Python checks first; pass only targeted evidence excerpts, file lists, schema refs and validation summaries to LLM calls.
- Trace rule: every action must cite source evidence, affected files, rollback or handoff path, and validation command.
...
|---|---|---|---|
| design | Define contracts, modules, UX/API boundaries, data shape and acceptance criteria. | contract schema check, acceptance criteria completeness | design artifact plus one validation signal for Copiloto Java Generico |
| build | Create scoped implementation plans or patches with affected files and rollback notes. | affected-file diff summary, lint/test command discovery | build artifact plus one validation signal for Copiloto Java Generico |
| test | Turn requirements and code risks into unit, integration, negative and regression tests. | test matrix expansion, pairwise/negative case generation | test artifact plus one validation signal for Copiloto Java Generico |
...
```json
{
  ...
  "title": "Copiloto Java Generico Output Contract",
  "type": "object",
  "required": [
"copilot_id",
"decision",
...
| Output contract | Produces one of: java_patch_plan, test_plan. | Returns generic consultancy text with no artifact. |
...
- The request enters a phase outside `Copiloto Java Generico` ownership.
