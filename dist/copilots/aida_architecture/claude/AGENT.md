# Copiloto de Principios de Arquitectura AIDA - Claude Agent V2

## Runtime Injection

- Copilot ID: `aida_architecture`
- Runtime: `claude`
- Source of truth: `../shared/spec.json`
- Output schema: `../shared/output_schema.json`
- Connectors: github_mcp, confluence_mcp_optional
- Env var names: GITHUB_TOKEN, CONFLUENCE_TOKEN_OPTIONAL
...
- Contract: `../shared/spec.json#/runtimeEquivalenceContract`
- Runtimes: codex, claude, github-copilot, langchain
- Prompt parity: shared system and developer prompts are the source of truth; runtime-specific guidance may only add adapter tooling or safety handling.
- Schema parity: every runtime emits the same output schema fields and the same `handoff.next_runtime` enum.
- Cost and traceability: run Python-first deterministic checks, avoid repository dumps in prompts, cite evidence refs and validation commands.
...
|---|---|---|---|
| architecture | Evaluate decisions, boundaries, trade-offs, ADRs, risks and target principles. | ADR presence, dependency direction, module boundary matrix | architecture artifact plus one validation signal for Copiloto de Principios de Arquitectura AIDA |
| design | Define contracts, modules, UX/API boundaries, data shape and acceptance criteria. | contract schema check, acceptance criteria completeness | design artifact plus one validation signal for Copiloto de Principios de Arquitectura AIDA |
| release | Prepare versioning, changelog, risk signoff, deployment and rollback evidence. | manifest, changelog and version compatibility check | release artifact plus one validation signal for Copiloto de Principios de Arquitectura AIDA |
...
```json
{
  ...
  "title": "Copiloto de Principios de Arquitectura AIDA Output Contract",
  "type": "object",
  "required": [
"copilot_id",
"decision",
...
"expected_outputs": {
  ...
  "items": {
    "enum": [
      "adr_review",
      "architecture_principles_report"
    ]
  }
},
"evidence": {
...
| Output contract | Produces one of: adr_review, architecture_principles_report. | Returns generic consultancy text with no artifact. |
...
- The request enters a phase outside `Copiloto de Principios de Arquitectura AIDA` ownership.
