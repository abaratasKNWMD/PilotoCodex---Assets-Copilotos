...
KB_REQUIRED_KB_PARTITION_FIELDS = [
    "kb_partition_map",
    "source_of_truth_registry",
    "context_window_budget",
    "excluded_sources",
...
def score(copilot: dict, request: str) -> tuple[float, list[str]]:
    ...
    if kb_audit_applies(copilot, words):
        score += CHEAP_PATH_THRESHOLD
        reasons.append("kb_audit:shared_contract")
...
def kb_audit_applies(copilot: dict, words: set[str]) -> bool:
    if copilot.get("id") != KB_AUDIT_COPILOT:
        return False
    if not words & KB_AUDIT_SCOPE:
        return False
    return bool(words & KB_AUDIT_REQUIRED)
...
def route_payload(index: dict, item: dict, raw_score: float, reasons: list[str]) -> dict:
    ...
    if item["id"] == KB_AUDIT_COPILOT:
        payload["kb_context_window_audit"] = kb_context_window_audit_evidence()
...
def kb_context_window_audit_evidence() -> dict:
    return {
        "mission": "Audits KB separation, source-of-truth rules and context windows.",
        "source_of_truth": KB_AUDIT_SOURCE,
        "policy_version": KB_AUDIT_VERSION,
        "required_evidence": KB_REQUIRED_EVIDENCE,
        "quality_gates": KB_QUALITY_GATES,
        "required_output_fields": [
            "copilot_id",
            "decision",
            "evidence",
        ...
        "required_kb_partition_fields": KB_REQUIRED_KB_PARTITION_FIELDS,
        "context_window_policy": {
            "max_evidence_refs": 12,
            "max_prompt_bytes": 12000,
            "summary_first": True,
            "full_document_only_when_gap_blocks_decision": True,
            "redaction_required": True,
            "overflow_action": "emit_gap_register_before_llm_escalation",
        },
        "runtime_equivalence": {
            "runtimes": list(RUNTIMES),
            "max_unexplained_drift": 0,
