# Runtime Contract - Copilots Manager
...
- Function: Discover copilots, install profiles, manage dependencies and route work.
...
Codex, Claude, GitHub Copilot Agents and LangChain use the same source, schema and trace rules.
...
- Adapter files: `codex/AGENT.md`, `claude/AGENT.md`, `github-copilot/copilot-agent.md`, `langchain/agent.py`
- Required trace fields: `copilot_id`, `decision`, `evidence`, `actions`, `validation`, `risks`
- Pairwise runtime drift: `maxUnexplainedDrift = 0`
- Prompt retention: store paths, summaries and digests only; expanded prompt bodies stay out of runtime matrices and handoff ledgers.
- Cost policy: deterministic catalog, schema, connector and evidence checks run before LLM judgement.
- Connector fallback: unavailable `github_mcp` access becomes pending evidence in the same schema-shaped output, not an invented connector result.
