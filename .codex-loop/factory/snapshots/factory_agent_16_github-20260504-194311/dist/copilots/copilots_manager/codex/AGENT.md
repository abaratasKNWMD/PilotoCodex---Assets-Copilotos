...
Codex local protocol: `../shared/codex_tool_protocol.json`.

- `workspace_boundary`: current workspace and declared files first.
- `snapshot_or_git`: Git metadata or `.codex-loop/rollback` snapshot before edits.
- `python_first`: deterministic local checks before judgement-heavy synthesis.
- `validation_commands`: run declared validators and report blockers.
- `secret_placeholders`: placeholders only for credentials, billing and customer data.
