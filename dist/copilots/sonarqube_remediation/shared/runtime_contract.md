...
See `output_schema.json`. Required top-level fields include `findings`, `runtime_trace` and `cost_budget` in addition to the base decision, evidence, actions, validation and risk fields. Each finding must carry severity, evidence reference, reproduction path and verification command.

## Runtime Parity Contract

- Shared contract: `spec.json#/runtimeParityContract`.
