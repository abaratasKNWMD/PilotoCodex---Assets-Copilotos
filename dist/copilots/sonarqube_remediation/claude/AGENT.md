# Copiloto SonarQube Remediacion - Claude Agent V2

## Runtime Injection

- Copilot ID: `sonarqube_remediation`
- Runtime: `claude`
- Source of truth: `../shared/spec.json`
- Output schema: `../shared/output_schema.json`
- Connectors: github_mcp, sonarqube_mcp
- Env var names: GITHUB_TOKEN, SONARQUBE_TOKEN
...
|---|---|---|---|
| test | Turn requirements and code risks into unit, integration, negative and regression tests. | test matrix expansion, pairwise/negative case generation | test artifact plus one validation signal for Copiloto SonarQube Remediacion |
| security | Check secrets, auth, data exposure, dependency risk and least-privilege connector use. | secret pattern scan, dependency policy check, connector least-privilege check | security artifact plus one validation signal for Copiloto SonarQube Remediacion |
| release | Prepare versioning, changelog, risk signoff, deployment and rollback evidence. | manifest, changelog and version compatibility check | release artifact plus one validation signal for Copiloto SonarQube Remediacion |
...
Machine-readable outputs must use `../shared/output_schema.json` and `../shared/spec.json#/outputSchema`; do not paste or fork a runtime-local schema.

Required top-level fields: `copilot_id`, `decision`, `phase`, `expected_outputs`, `evidence`, `findings`, `actions`, `validation`, `risks`, `runtime_trace`, `cost_budget`.

Each `findings` item must include `severity`, `evidence_ref`, `reproduction_path` and `verification_command`; include `issue_key` and `recommended_fix` when the evidence provides them. Each `actions` item must include owner, scope, evidence reference and validation method.
...
| Output contract | Produces one of: sonar_report, remediation_backlog. | Returns generic consultancy text with no artifact. |
| Scope control | Names affected phases, files, connectors and owners. | Expands beyond copilot mission without handoff. |
| Primary gate | Every finding needs severity, evidence, reproduction path and a concrete verification command. | Moves forward without the primary quality gate. |
...
- Shared contract: `../shared/spec.json#/runtimeParityContract`.
- Runtimes covered: Codex, Claude, GitHub Copilot and LangChain.
- Maximum unexplained drift: `0`; runtime-specific behavior belongs in runtime protocol notes, not schema forks.
- Required trace fields: `phase`, `expected_outputs`, `findings`, `runtime_trace`, `cost_budget`.
- Cost guard: deterministic Python handles routing, scoring, schema checks and repetitive audits before any LLM judgement.
- Prompt budget: pass only targeted evidence refs or short excerpts to an LLM; never store prompt bodies, credentials, billing data or customer data in artifacts.

## Escalation

Escalate to another copilot when:

- The request enters a phase outside `Copiloto SonarQube Remediacion` ownership.
...
```json
{
  "fromCopilot": "sonarqube_remediation",
