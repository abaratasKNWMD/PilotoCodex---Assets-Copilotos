...
KB separation is not accepted unless `python tools/validate_copilot_factory.py` can find the `factory_agent_21_kb` contract in a shared artifact, the Firefly v6 runtime adapters, and the semantic-router evidence payload.
...
`dist/copilots/firefly_v6/shared/kb_context_window_audit.json` is the source of truth for KB separation, source-of-truth rules and context-window budgets. Runtime adapters may cite that artifact, but must not redefine the policy independently.

The KB audit requires these evidence fields before LLM escalation: `kb_partition_map`, `source_of_truth_registry`, `context_window_budget` and `runtime_trace`.

Context windows stay bounded: cite up to 12 evidence refs, summarize first, redact secret-like values and emit a gap register before sending larger source material to an LLM runtime.
